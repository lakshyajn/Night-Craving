      ...
      {isCartOpen && (
        <AnimatePresence>
          ...
          <motion.div
            ...
            animate={{ y: 0 }}
            exit={{ y: '100%' }}
            className="fixed bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-lg z-50 max-h-[80vh] flex flex-col"
          >
            {/* HEADER */}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {cart.length === 0 ? (
                ...
                <>
                  {cart.map((item, index) => (
                    <motion.div
                      key={`${item.id}-${index}-${JSON.stringify(item.selectedAddons)}`}
