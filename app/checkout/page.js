...
const validateForm = (formValues) => {
  ...
  return requiredFields.filter(field => !formValues[field]?.trim());
};
...
const createWhatsAppMessage = (orderDetails) => {
  ...
  const itemsDetail = items.map(item => {
    const addonsText = item.selectedAddons?.length
      ? `\n    Add-ons: ${item.selectedAddons.map(addon => `${addon.name} (₹${addon.price})`).join(', ')}`
      : '';
    return `*${item.name} x ${item.quantity}*${addonsText}`;
  }).join('\n\n');

  return `Hello ${userName}, Your order details:\n\n# After10-OID: ${orderId}\n\nOrder details: \n${itemsDetail}\n\n---------------------------------\n💰 Subtotal: ₹${totalAmount}\n🚚 Delivery Charge: ₹0\n---------------------------------\n💵 Grand Total: ₹${totalAmount} INR\n---------------------------------\nDelivery Address:\n${address.name}\n📱 ${address.contactNo}\n📍 ${address.line1}\n   ${address.line2 || ''}`;
};
...
export default function CheckoutPage() {
  ...
  const orderId = useMemo(() => `A10-${Date.now().toString().slice(-6)}`, []);
  ...
  const generateWhatsAppLink = useCallback((_, message) => {
  ...
  const handlePlaceOrder = useCallback(() => {
    ...
    const message = createWhatsAppMessage(details);
    const whatsappLink = generateWhatsAppLink(formValues.Mobile_No, message);

    clearCart();
    window.open(whatsappLink, '_blank');
    router.push('/');
  }, [formValues, cart, totalAmount, orderId, generateWhatsAppLink, clearCart, router]);
